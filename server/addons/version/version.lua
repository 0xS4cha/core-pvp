    ...
    PerformHttpRequest(versions_url, function(status, response_text)
        if status == 200 then
            ...
            if not versions_data or not versions_data.resources then
                print("Error decoding JSON or invalid structure for resource: " .. resource_name)
            ...
            if resource_info and resource_info.version then
                ...
                if err then
                    Console.Version("Error comparing versions for '" .. resource_name .. "': " .. err .. "^7")
                    if opts.callback then opts.callback(false, "Error comparing versions: " .. err) end
                    return
                end
                if is_outdated then
                    Console.Version("^7Update available for resource: '^5" .. resource_name .. "^7'. Current version: ^2" .. current_version .. " ^7Latest version: ^2" .. latest_version .. "^7.")
                ...
                Console.Version("Version info for '" .. resource_name .. "' not found.^7")
                if opts.callback then opts.callback(false, "Version info not found for resource.") end
            end
        else
            Console.Version("HTTP Error: " .. tostring(status) .. " while fetching version data for " .. resource_name)
...
local opts = {
    ...
    url_path = 'syroneV/fivem_resource_version/blob/main/versions.json',
}
check_version(opts)