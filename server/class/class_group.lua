...
Group = {
    ...
    getGroupRankId = function(groupId, rank)
        local result = MySQL.Sync.fetchScalar("SELECT * FROM group_rank WHERE group_rank.id_group= @groupId and `rank` = @rank"
            , {
            ['@groupId'] = groupId,
            ['@rank'] = rank
        })
    ...
    CreateGroup = function(source, name, rgb, description, rank)
        ...
        if not Group.DoesGroupExist(name) then
            local result = MySQL.Sync.insert("INSERT INTO group_list (`name`, `description`, `rgb`) VALUES (@name, @description,  @rgb)"
                , {
                ['@name'] = name,
                ['@rgb'] = json.encode(rgb),
    ...
    createRank = function(groupName, name, rank)
        if Group.DoesGroupExist(groupName) then
            local result = MySQL.Sync.insert("INSERT INTO group_rank (name, rank, id_group, permission) VALUES (?, ?, ?, ?)"
                , {
                    name,
                    rank,
    ...
    setPermRank = function(groupName, rank, perm)
        if Group.DoesGroupExist(groupName) then
            local result = MySQL.Sync.execute("UPDATE group_rank SET perm = @perm WHERE id_group = @groupId AND `rank` = @rank"
                , {
    ...
    addPlayerInGroup = function(playerId, groupId, rank)
    ...
    changePlayerRankInGroup = function(playerId, groupId, rank)
        MySQL.Async.execute("UPDATE group_members SET id_rank = @rankId WHERE id_group = @groupId AND id_player = @playerId"
            , {
                ['@groupId'] = groupId,
                ['@id_player'] = playerId,
    ...
    getPlayerGroup = function(playerId)
        local result = MySQL.Sync.fetchAll("SELECT * FROM group_members LEFT JOIN group_list ON group_members.id_group = group_list.id_group WHERE group_members.id_player = @playerId", {
            ['@playerId'] = playerId
        })
        if result[1] ~= nil then
    ...
    getAllGroupMembersBygroupName = function(groupName)
        if Group.DoesGroupExist(groupName) then
            local result = MySQL.Sync.fetchAll("SELECT * FROM group_members LEFT JOIN players ON group_members.id_player = players.id LEFT JOIN group_rank ON group_members.id_rank = group_rank.id WHERE group_members.id_group = @groupId"
                , {
                ['@groupId'] = Group.getGroupId(groupName)
            })
            if result ~= nil then
    ...
    getAllGroupGradeBygroupName = function(groupName)
        if Group.DoesGroupExist(groupName) then
            local result = MySQL.Sync.fetchAll("SELECT * FROM group_list LEFT JOIN group_rank ON group_list.id = group_rank.id_group WHERE group_list.id = @groupId"
                , {
                ['@groupId'] = Group.getGroupId(groupName)
            })
            if result ~= nil then
