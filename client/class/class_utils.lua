...
function Utils.Teleport(entity, coords)
...
function Utils.InputString(text, number, windows)
...
function Utils.StartMusicEvent(event)
    PrepareMusicEvent(event)
    return TriggerMusicEvent(event) == 1
end


function Utils.LoadIsland(isIsland)
...
function Utils.GetRankIcon(level)
	local level = (level)
	if level < 10 then
...
function Utils.GetKillerPedFromSource(source)
...
function Utils.GetPedCauseOfDeath(victim, killer)
	local weapon
