Cam = {
    ...
    HandleSmartDof = function(name, entity, dofValue) 
        if Cam.cams[name] ~= nil then
            ...
            local nearDof = distance - 1.0
            local farDof = distance + 1.0

            SetUseHiDof()
            SetCamNearDof(Cam.cams[name], nearDof)
            SetCamFarDof(Cam.cams[name], farDof)
            SetCamUseShallowDofMode(Cam.cams[name], true)
            SetCamDofStrength(Cam.cams[name], dofValue)
            
            SetCamDofMaxNearInFocusDistance(Cam.cams[name], nearDof)
            SetCamDofMaxNearInFocusDistanceBlendLevel(Cam.cams[name], nearDof)
