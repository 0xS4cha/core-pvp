...
local function trigger(eventName, source, ...)
    ...
    triggerNet("sxGroup:callback:request", source, eventName, currentRequestId, ...);
	return Citizen.Await(p);
end

RegisterNetEvent('sxGroup:callback:request', function(eventName, requestId, ...)
    local src = source;
    if (handlers[eventName]) then
        local result = handlers[eventName](src, ...);
        triggerNet('sxGroup:callback:response', src, requestId, result);
        return;
    end
    print(("Attempt to trigger an unregistered event: '%s'"):format(eventName));
end);

RegisterNetEvent('sxGroup:callback:response', function(requestId, result)
    local src = source;
