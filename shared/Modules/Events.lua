...
NetworkEvents = Class.new(function(class)
        ...
        RegisterNetEvent(eventName, function(...)
            ...
            if (IsDuplicityVersion()) then
                ...
                if (self.rate[src][eventName] >= 10) then
                    ...
                    Logger:info('ANTICHEAT', ("Player spamming event (playerId: %s, event: %s)"):format(src, eventName))
    ...
    function self:OnProtectedNet(eventName, callback)
        ...
        if (not IsDuplicityVersion()) then
            return Logger:error('ANTICHEAT', ("Shared.Events:OnProtectedNet('%s'): Protected event can be used only on server side. Aborting..."):format(eventName));
        end
