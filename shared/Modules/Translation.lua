...
function RegisterDefaultPhrases(phrases)
	...
	Logger:info('TRANSLATION',"Register default translation")
end
...
function AddLanguage(name, tbl)
	...
	Logger:info('TRANSLATION',"Register", name, 'translation')
end
...
function GetPhrase(name, ...)
	...
	if defaultPhrases[name] then
		return defaultPhrases[name] and string.format(defaultPhrases[name], table.unpack(arguments)) or (#arguments > 0 and string.format(name, table.unpack(arguments)) or name) or nil
	end
...
local function getMissingPhrases(ply, args, cmd)
    if not args[1] then Logger:info('TRANSLATIOn', "Please run the command with a language code e.g. darkrp_getphrases \"" .. mainLanguage .. "\"") return end
    local lang = registeredLanguages[args[1]]
    if not lang then Logger:warn('TRANSLATION', "This language does not exist! Make sure the casing is right.")
        Logger:info('TRANSLATION', "Available languages:")
        for k in pairs(registeredLanguages) do print(k) end
        return
    end

    Logger:info('TRANSLATION', GetMissingPhrases(args[1]))
end
